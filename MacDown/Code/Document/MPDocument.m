...
-(void) updateHeaderLocations
{
    ...
    NSString *script = @"(function() {"
        ...
        "    } else if (tagName === 'IMG') {"
        "        var imgDesc = (node.alt || node.src || '').substring(0, 30);"
        "        allImages.push(imgDesc);"
        "        var parent = node.parentElement;"
        "        var isStandalone = false;"
        "        var reason = '';"
        "        "
        "        // Check if image is the only child of its parent"
        "        if (parent && parent.children.length === 1) {"
        "            isStandalone = true;"
        "            reason = 'direct';"
        "        }"
        "        // Also check if image is in a link that's the only child of its parent (clickable image)"
        ...
        "                reason = 'in-link';"
        "            } else {"
        "                console.log('SKIPPED IMG (in link, but grandparent has ' + (grandparent ? grandparent.children.length : 0) + ' children): ' + imgDesc);"
        "            }"
        "        } else {"
        "            console.log('SKIPPED IMG (parent: ' + (parent ? parent.tagName : 'none') + ', children: ' + (parent ? parent.children.length : 0) + '): ' + imgDesc);"
        "        }"
        "        "
        "        if (isStandalone) {"
        "            result.push(node);"
        "            debug.push('IMG(' + reason + '): ' + imgDesc);"
        "        }"
        "    }"
        "}"
        "console.log('Total images found in DOM: ' + allImages.length + ' = ' + allImages.join(', '));"
