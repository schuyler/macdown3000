        ...
        if (bodyNodes.length >= 1)
        {
            ...
            if (result && [result rangeAtIndex:1].location != NSNotFound)
            {
                ...
                NSString *updateScript = [NSString stringWithFormat:
                    ...
                    @"  });"
                    @"  "
                    @"  Promise.race([Promise.all(imagePromises), timeoutPromise]).then(function() {"
...
-(void) updateHeaderLocations
{
    ...
    NSString *script = @"(function() {"
        ...
        "                    if (grandparent.children[j].tagName === 'IMG' || grandparent.children[j].tagName === 'A') {"
        "                        var node = grandparent.children[j];"
        "                        if (node.tagName === 'IMG' || (node.children.length === 1 && node.children[0].tagName === 'IMG')) {"
        "                            imgCount++;"
        "                        }"
        "                    }"
        "                }"
