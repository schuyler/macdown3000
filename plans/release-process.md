...
**Re-signing must re-apply entitlements.** After the Xcode build, the release workflow signs nested components inside-out — the bundled `macdown` CLI binary, Sparkle's nested components (see below), then the Quick Look extension — and re-signs the app bundle last to re-seal it. It deliberately does not use `--deep`, which would apply the app's entitlements to every nested component and strip the Quick Look extension's sandbox entitlement (issue #284). A `codesign` re-sign *replaces* the existing signature, so it must pass `--entitlements MacDown/MacDown.entitlements` — otherwise the shipped app loses its entitlements entirely and `NSSavePanel` (Move To / Save As…) fails with ViewBridge error 14 on macOS Sequoia and later. To prevent this regression from shipping silently, the pipeline asserts the `com.apple.security.files.user-selected.read-write` entitlement is present at three gates: after the re-sign and on the app inside the DMG (`release.yml`), and at the final pre-publish verification (`staple-release.yml`).

**Sparkle's nested components must be re-signed.** Sparkle ships from CocoaPods ad-hoc signed, and the pod's `[CP] Embed Pods Frameworks` phase re-signs only the framework's top level — without hardened runtime and without a secure timestamp. Notarization rejects that on all three counts. `Tools/sign_sparkle.sh` re-signs the XPC services, `Autoupdate`, `Updater.app`, and the framework itself inside-out with the release identity, `--options runtime`, and `--timestamp`; the app bundle re-sign that follows re-seals over it. `codesign --deep --strict` does not catch this, because it verifies structural validity rather than the signer identity of nested code — so `Tools/verify_sparkle_signature.sh` asserts Developer ID authority, hardened runtime, team identifier, and a secure timestamp on every component at the same three gates as the entitlement checks (issue #553).
